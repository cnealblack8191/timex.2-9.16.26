...
begin
  if tg_op = 'INSERT' then
    ...
    v_old := to_jsonb(old);
    v_new := to_jsonb(new);
    -- photo housekeeping (storage delete/update writes) is not a real change
    if old.clock_in is not distinct from new.clock_in
       and old.clock_out is not distinct from new.clock_out
       and old.manual_hours is not distinct from new.manual_hours
       and old.job_id is not distinct from new.job_id
       and old.entry_type is not distinct from new.entry_type
       ...
       and old.edited is not distinct from new.edited
       and old.after_close is not distinct from new.after_close then
      return new;
    end if;
    if old.voided is distinct from new.voided then
      v_action := case when new.voided then 'void' else 'unvoid' end;
    else
      v_action := 'update';
    end if;
    v_via := coalesce(new.source, 'unknown');
    if v_uid is not null and v_via not in ('portal', 'time-card', 'kiosk', 'mobile') then
      v_via := 'portal';
    end if;
  end if;
  ...
  insert into public.time_entry_revisions (
    ...
    v_action,
    v_uid,
    case when v_uid is not null then public.actor_name() else null end,
    v_via,
    v_old,
    v_new
  );
